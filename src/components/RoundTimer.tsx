...
export default function RoundTimer({ round }: RoundTimerProps) {
  ...
  const statusColors = {
    Open: { text: '#10b981', bg: 'rgba(16, 185, 129, 0.15)', border: 'rgba(16, 185, 129, 0.3)', dot: '#10b981' },
    Full: { text: '#f59e0b', bg: 'rgba(245, 158, 11, 0.15)', border: 'rgba(245, 158, 11, 0.3)', dot: '#f59e0b' },
    Settled: { text: '#7C3AED', bg: 'rgba(124, 58, 237, 0.15)', border: 'rgba(124, 58, 237, 0.3)', dot: '#7C3AED' },
  };
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
        {/* Round number */}
        <div>
          <div className="text-xs text-slate-500 uppercase tracking-wider mb-0.5">Current Round</div>
          <div
          ...
          >
            <span
              ...
              style={{
                ...
                animation: round.status === 'Open' ? 'pulse 2s ease-in-out infinite' : 'none',
              }}
            />
            {round.status === 'Open' ? 'LIVE' : round.status === 'Full' ? 'DRAWING SOON' : 'SETTLED'}
          </div>
          {timeText && (
            <div className="text-xs text-slate-500">{timeText}</div>
          )}
        </div>
      </div>
      ...
      <div>
        <div className="flex justify-between text-xs mb-2">
          ...
          <span
            className="font-bold"
            style={{ color: round.slotsFilled === round.slots ? '#f59e0b' : '#10b981' }}
          >
        ...
        >
          <div
            ...
            style={{
              width: `${filledPercent}%`,
              background: round.slotsFilled === round.slots
                ? 'linear-gradient(90deg, #f59e0b, #FFD700)'
                : 'linear-gradient(90deg, #7C3AED, #10b981)',
              boxShadow: round.slotsFilled === round.slots
        ...
        <div className="flex gap-1 mt-2">
          {Array.from({ length: round.slots }).map((_, i) => (
            <div
              ...
              style={{
                background: i < round.slotsFilled
                  ? round.slotsFilled === round.slots ? '#f59e0b' : '#7C3AED'
                  : 'rgba(255,255,255,0.1)',
                boxShadow: i < round.slotsFilled
      ...
      <div className="mt-3 pt-3" style={{ borderTop: '1px solid rgba(124, 58, 237, 0.1)' }}>
        ...
        <div className="flex gap-4 mt-1.5">
          ...
          <div className="text-xs text-slate-500">
            ...
            {((Number(round.pool) * 0.94 * 0.15) / 1_000_000_000).toFixed(3)} SOL
          </div>
          <div className="text-xs text-slate-500">
            <span className="text-amber-700 font-medium">3rd:</span>{' '}
            {((Number(round.pool) * 0.94 * 0.125) / 1_000_000_000).toFixed(3)} SOL
          </div>
        </div>
      </div>
    </div>
